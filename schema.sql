...
CREATE TABLE settings (id INT PRIMARY KEY DEFAULT 1, company_name VARCHAR(120), manager_name VARCHAR(120), manager_email VARCHAR(120), manager_whatsapp VARCHAR(32), whatsapp_phone_id VARCHAR(64), whatsapp_token TEXT, whatsapp_template VARCHAR(120) DEFAULT 'payment_receipt', smtp_host VARCHAR(120), smtp_port INT, smtp_user VARCHAR(120), smtp_pass TEXT, from_email VARCHAR(120), from_name VARCHAR(120), timezone VARCHAR(64) DEFAULT 'Asia/Dubai', created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
