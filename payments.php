...
require_login();
$pdo = db();
$tenants = $pdo->query('SELECT t.*, u.unit_no, u.monthly_rent_aed FROM tenants t LEFT JOIN units u ON u.id=t.unit_id')->fetchAll();
...
$payments = $pdo->query('SELECT p.*, t.full_name, u.unit_no FROM payments p JOIN tenants t ON t.id=p.tenant_id JOIN units u ON u.id=t.unit_id ORDER BY p.created_at DESC LIMIT 200')->fetchAll();
$csrf = csrf_token();
?>
...
<form method='post' class='grid grid-cols-3 gap-2'>
  ...
  <select name='tenant_id' class='border p-2 rounded col-span-1'>
    <?php foreach ($tenants as $t): ?>
    <option value='<?= $t['id'] ?>'><?= htmlspecialchars($t['full_name']) ?> - <?= htmlspecialchars($t['unit_no'] ?? '') ?></option>
    <?php endforeach; ?>
  </select>
...
<thead class='bg-gray-100'><tr><th class='p-2'>Date</th><th class='p-2'>Tenant</th><th class='p-2'>Unit</th><th class='p-2'>Period</th><th class='p-2'>Amount</th><th class='p-2'>Receipt</th></tr></thead>
<tbody>
...
<tr class='border-t'>
  ...
  <td class='p-2'><?= number_format($p['amount_aed'], 2) ?></td>
  <td class='p-2'><a href='receipt.php?id=<?= intval($p['id']) ?>' class='text-blue-600 hover:underline' target='_blank'>🖨 Receipt</a></td>
</tr>
<?php endforeach; ?>
</tbody></table>
</div>
